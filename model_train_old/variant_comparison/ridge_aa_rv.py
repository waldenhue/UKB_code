...
if __name__ == "__main__":
    ...
    all_metrics = estimate_model(ridge_pipeline, X_test, y_test)
    carriers_metrics = estimate_model(ridge_pipeline, X_test_rv, y_test_rv)

    list1 = ['AA','ridge','cv+rv','all',all_metrics['r2'],all_metrics['mse']]
    list2 = ['AA','ridge','cv+rv','carriers',carriers_metrics['r2'],carriers_metrics['mse']]
    list3 = ['AA','ridge','cv+rv',best_alpha]
    ...
    ## output results
    df_report.to_csv('./report/ridge_aa_rv.csv')
    df_para.to_csv('./para/ridge_aa_rv.csv')
