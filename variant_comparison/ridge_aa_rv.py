...
if __name__ == "__main__":
    ...
    plt.savefig('/data/med-hudh/learning/ridge_learning_curve_aa_rv.png')
    plt.close()
    ...
    y_pred_carriers = ridge_pipeline.predict(X_test_m1)
    test_score_carriers = r2_score(y_test_m1, y_pred_carriers)
    mse1 = mean_squared_error(y_test_m1, y_pred_carriers)
    list1 = ['AA','ridge','cv+rv','all',test_score,mse0]
    list2 = ['AA','ridge','cv+rv','carriers',test_score_carriers,mse1]
    ...
    df_report.to_csv('/data/med-hudh/report/ridge_aa_rv.csv')
    df_para.to_csv('/data/med-hudh/para/ridge_aa_rv.csv')
