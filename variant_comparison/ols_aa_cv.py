...
if __name__ == "__main__":
    ...
    plt.savefig('/data/med-hudh/learning/linear_learning_curve_aa_all.png')  
    plt.close()
    ...
    y_pred_carriers2 = linear_pipeline.predict(X_test_rv)
    test_score_carriers2 = r2_score(y_test_rv, y_pred_carriers2)
    mse_carrers = mean_squared_error(y_test_rv, y_pred_carriers2)
    list1 = ['AA','linear','cv','all',test_score,mse_all]
    list2 = ['AA','linear','cv','carriers',test_score_carriers2,mse_carrers]
    df_report = pd.DataFrame([list1,list2],columns=['Trait','Model','Variant','Group','R2','MSE'])

    df_report.to_csv('/data/med-hudh/report/linear_aa_all.csv')
